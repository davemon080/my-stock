...
const App: React.FC = () => {
  ...
  const initApp = async () => {
    ...
    try {
      ...
      showToast("Access Link Disturbed", "error");
    } finally {
  ...
  const handleAiAnalysis = async () => {
    ...
    try {
      ...
      showToast("AI Processor Busy", "error");
    } finally {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (loginRole === 'Admin') {
      if (loginPassword === config.adminPassword) {
        ...
        setLoginError('Invalid Credential Signature');
      }
    } else {
      ...
      if (seller) {
        ...
        showToast(`Terminal Activated for ${seller.name}`, "success");
      } else {
        setLoginError('Staff Key Incorrect');
      }
    }
  };
  ...
  const handleSaveProduct = async (data: Omit<Product, 'id' | 'lastUpdated' | 'sku'>) => {
    ...
    showToast("Entry Registered", "success");
  };
  ...
  const deleteProduct = (id: string) => {
    setConfirmModal({
      ...
      message: "This SKU and all associated data will be removed from the local branch records.",
      onConfirm: async () => {
  ...
  const handleWipeDatabase = () => {
    setConfirmModal({
      isOpen: true,
      title: "Permanent System Reset?",
      message: "This will restore the system to factory settings. All branches, accounts, and historical records will be permanently erased.",
      onConfirm: async () => {
        setConfirmModal({
          isOpen: true,
          title: "Final Authorization Required",
          message: "You are initiating a global system wipe. This action is terminal and cannot be reversed. Proceed?",
          onConfirm: async () => {
            setConfirmModal(null);
            setIsInitializing(true);
            try {
              ...
              showToast("Reset Sequence Aborted", "error");
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        showToast("Visual Branding Staged", "info");
      };
        ...
        <div className="relative z-10 flex flex-col items-center gap-12 animate-in fade-in duration-1000">
          ...
          <div className="flex flex-col items-center gap-4">
             <div className="h-1 w-48 bg-white/5 rounded-full overflow-hidden relative">
                <div className="absolute inset-y-0 left-0 bg-gradient-to-r from-blue-600 to-indigo-600 animate-loading-bar rounded-full shadow-[0_0_15px_rgba(59,130,246,0.5)]"></div>
             </div>
             <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] translate-x-[0.2em] animate-pulse">Establishing Secure Session</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="w-full max-w-md bg-white rounded-[3rem] p-10 shadow-2xl relative z-10 animate-in fade-in zoom-in-95 duration-500">
          <div className="text-center mb-10">
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-4 italic">Enterprise Network Hub</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            {loginRole === 'Seller' && (
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Staff Identifier</label>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Access Signature</label>
            ...
            <button type="submit" className="w-full py-5 bg-blue-600 text-white rounded-3xl font-black uppercase tracking-widest shadow-xl shadow-blue-600/30 hover:bg-blue-700 transition-all active:scale-95">Verify Identity</button>
          </form>
        </div>
      </div>
    );
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
           <div className="w-full max-w-sm bg-white rounded-[2.5rem] p-10 shadow-2xl">
              ...
              <div className="flex gap-4">
                 ...
                 <button onClick={confirmModal.onConfirm} className="flex-1 py-4 bg-rose-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-rose-600/20">Authorize</button>
              </div>
           </div>
        </div>
      )}
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-72 bg-slate-950 text-white flex flex-col transition-all duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="p-6 shrink-0 border-t border-white/5">
           <div className="p-5 bg-white/5 rounded-3xl mb-4">
              <p className="text-[8px] font-bold text-slate-500 uppercase tracking-widest mb-1 italic">Authenticated</p>
              <p className="text-sm font-black truncate uppercase">{currentUser.name}</p>
           </div>
           <button onClick={handleLogout} className="w-full py-3 bg-rose-500/10 text-rose-500 text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-rose-500 hover:text-white transition-all active:scale-95">Terminate Session</button>
        </div>
      </aside>
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-10 custom-scrollbar">
              ...
              <div className="bg-white rounded-[3rem] p-10 border border-slate-200 shadow-sm">
                ...
                {aiInsights ? (
                  ...
                  <div className="py-20 text-center text-slate-300 font-black uppercase tracking-[0.2em] italic border-2 border-dashed rounded-[3rem] border-slate-100">
                    Run Analysis to Synchronize with Strategic Advisor
                  </div>
                )}
              </div>
            </div>
             ...
             <div className="max-w-7xl mx-auto space-y-6">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                   <div className="relative w-full max-w-md">
                      ...
                      <input type="text" placeholder="Search Master File..." className="w-full pl-12 pr-6 py-3 bg-white border-2 border-slate-100 rounded-2xl focus:border-blue-600 outline-none font-bold shadow-sm transition-all" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                   </div>
                   {currentUser.role === 'Admin' && (
                     <button onClick={() => { setEditingProduct(null); setIsModalOpen(true); }} className="px-8 py-3 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-blue-600/30 active:scale-95 transition-all">New Entry</button>
                   )}
                </div>
                   ...
                   <table className="w-full text-left min-w-[800px]">
                      <thead className="bg-slate-50 border-b">
                         <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            ...
                            <th className="px-10 py-6">SKU Signature</th>
                      ...
                      <tbody className="divide-y divide-slate-100">
                         ...
                         {filteredProducts.length === 0 && (
                           <tr>
                             <td colSpan={5} className="py-20 text-center text-slate-300 font-black uppercase tracking-widest italic">Registry Empty</td>
                           </tr>
                         )}
                      </tbody>
                   </table>
          ...
          {activeTab === 'Register' && (
            <div className="max-w-7xl mx-auto space-y-8">
               <div className="relative max-w-2xl mx-auto">
                  ...
                  <input type="text" placeholder="Catalog Quick Search..." className="w-full pl-16 pr-8 py-5 text-md font-bold bg-white border-2 border-transparent rounded-[2.5rem] focus:border-blue-600 focus:bg-white outline-none shadow-sm transition-all" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
               </div>
            ...
            <div className="max-w-7xl mx-auto space-y-6">
               <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="relative w-full max-md">
                     <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"><ICONS.Search /></span>
                     <input type="text" placeholder="Locate Record..." className="w-full pl-12 pr-6 py-3 bg-white border-2 border-slate-100 rounded-2xl focus:border-blue-600 outline-none font-bold shadow-sm" value={searchTermTransactions} onChange={e => setSearchTermTransactions(e.target.value)} />
                  </div>
               </div>
                  ...
                  <table className="w-full text-left min-w-[800px]">
                     <thead className="bg-slate-50 border-b">
                        <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                           ...
                           <th className="px-10 py-6">Authorization Value</th>
                           <th className="px-10 py-6 text-right">Actions</th>
                        </tr>
                     </thead>
                     <tbody className="divide-y divide-slate-100">
                        {activeBranchTransactions.filter(t => 
                          ...
                          <tr key={t.id} className="hover:bg-blue-50/10 transition-colors">
                             ...
                             <td className="px-10 py-6 text-right">
                                <div className="flex justify-end gap-2">
                                  <button onClick={() => setReceiptToShow(t)} title="Inspect Record" className="p-2.5 bg-slate-100 text-slate-500 rounded-xl hover:bg-blue-600 hover:text-white transition-all active:scale-95">
                                  ...
                                  <button onClick={() => { setReceiptToShow(t); setTimeout(() => window.print(), 200); }} title="Hard Copy" className="p-2.5 bg-slate-100 text-slate-500 rounded-xl hover:bg-emerald-600 hover:text-white transition-all active:scale-95">
          ...
          {activeTab === 'Revenue' && currentUser.role === 'Admin' && (
             <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                   <StatCard title="Gross Settlement" value={`₦${activeBranchTransactions.reduce((acc, t) => acc + t.total, 0).toLocaleString()}`} icon={<ICONS.Revenue />} color="emerald" />
                   <StatCard title="Resource Cost" value={`₦${activeBranchTransactions.reduce((acc, t) => acc + t.totalCost, 0).toLocaleString()}`} icon={<ICONS.Inventory />} color="amber" />
                   ...
                   <StatCard title="Efficiency Index" value={`${((activeBranchTransactions.reduce((acc, t) => acc + (t.total - t.totalCost), 0) / (activeBranchTransactions.reduce((acc, t) => acc + t.total, 0) || 1)) * 100).toFixed(1)}%`} icon={<ICONS.Register />} color="slate" />
                </div>
                <div className="bg-white rounded-[3rem] p-10 border border-slate-200 shadow-sm overflow-hidden">
                   <h3 className="text-xl font-black uppercase mb-8 italic">Financial Ledger</h3>
                   <div className="overflow-x-auto">
                      <table className="w-full text-left min-w-[700px]">
                         <thead className="bg-slate-50 border-b">
                            <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                               <th className="px-8 py-5">Event Time</th>
                               <th className="px-8 py-5">Settle Value</th>
                               <th className="px-8 py-5">Net Gain</th>
                               <th className="px-8 py-5 text-right">Ratio</th>
                            </tr>
                         </thead>
                         <tbody className="divide-y divide-slate-100">
            ...
            <div className="max-w-4xl mx-auto space-y-10 pb-40 animate-in fade-in duration-500">
               ...
               <div className="bg-white rounded-[3rem] p-10 border border-slate-200 shadow-sm">
                  ...
                  <form onSubmit={async (e) => {
                    ...
                    form.reset();
                    showToast("Network Node Established", "success");
                  }} className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                    <input name="branchName" required placeholder="Node Alias" className="px-6 py-4 bg-slate-50 border-2 rounded-2xl outline-none font-bold focus:border-blue-600 focus:bg-white transition-all" />
                    <input name="branchLoc" required placeholder="Spatial Coordinates" className="px-6 py-4 bg-slate-50 border-2 rounded-2xl outline-none font-bold focus:border-blue-600 focus:bg-white transition-all" />
                    <button type="submit" className="sm:col-span-2 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest active:scale-95 transition-all">Register Node</button>
                  </form>
                  
                  <div className="space-y-3">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-4">Active Topology</p>
                    {config.branches.map(b => (
                          ...
                          <button 
                            onClick={() => {
                              setConfirmModal({
                                ...
                                message: `Shutdown sequence for '${b.name}'? All local data will be purged.`,
                                onConfirm: async () => {
                                  ...
                                  setConfirmModal(null);
                                  showToast("Node Link Terminated", "info");
                                }
                              });
                            }}
               ...
               {/* Staff Management */}
               <div className="bg-white rounded-[3rem] p-10 border border-slate-200 shadow-sm">
                  <h3 className="text-2xl font-black mb-8 uppercase tracking-tight text-slate-500">Personnel Deployment</h3>
                  <form onSubmit={async (e) => {
                    e.preventDefault();
                    const form = e.target as HTMLFormElement;
                    const fd = new FormData(form);
                    ...
                    form.reset();
                    showToast(`Personnel ${seller.name} deployed`, "success");
                  }} className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                    <input name="staffName" required placeholder="Personnel Name" className="px-6 py-4 bg-slate-50 border-2 rounded-2xl font-bold focus:border-blue-600 focus:bg-white transition-all" />
                    <input name="staffEmail" required type="email" placeholder="Credential ID" className="px-6 py-4 bg-slate-50 border-2 rounded-2xl font-bold focus:border-blue-600 focus:bg-white transition-all" />
                    <input name="staffPin" required placeholder="Secure Pin" className="px-6 py-4 bg-slate-50 border-2 rounded-2xl font-bold focus:border-blue-600 focus:bg-white transition-all" />
                    ...
                    <button type="submit" className="sm:col-span-2 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest active:scale-95 transition-all">Authorize Signature</button>
                  </form>
                  
                  <div className="space-y-3">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-4">Field Personnel</p>
                    {config.sellers.map(s => (
                      <div key={s.id} className="p-5 bg-slate-50 border border-slate-100 rounded-[2rem] flex items-center justify-between group hover:bg-white hover:border-blue-200 transition-all">
                        ...
                        <button onClick={async () => {
                          ...
                          const sellers = await db.getSellers();
                          setConfig({...config, sellers});
                          showToast("Authorization Revoked", "info");
                        }} className="p-3 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all active:scale-90">
                          <ICONS.Trash />
                        </button>
                      </div>
               ...
               <div className="bg-white rounded-[3rem] p-10 border border-slate-200 shadow-sm">
                  <h3 className="text-2xl font-black mb-8 uppercase tracking-tight text-slate-500">Branding Interface</h3>
                  <div className="space-y-8">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-8 p-6 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200">
                      ...
                      <div className="flex-1 text-center sm:text-left">
                        <p className="text-sm font-black text-slate-900 mb-1 uppercase italic">Identity Signature</p>
                        <p className="text-xs text-slate-500 mb-4 font-medium uppercase tracking-tighter">Geometric Profile Image Recommended</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div>
                         <label className="text-[10px] font-black text-slate-400 uppercase block mb-2">Network Label</label>
                      ...
                      <div>
                         <label className="text-[10px] font-black text-slate-400 uppercase block mb-2">Global Master Key</label>
                      ...
                      }} className="sm:col-span-2 py-5 bg-indigo-600 text-white rounded-[2rem] font-black uppercase text-[10px] tracking-widest shadow-xl shadow-indigo-600/20 active:scale-95 transition-all">Deploy Configuration</button>
                    </div>
                  </div>
               </div>
               ...
               <div className="bg-rose-50/30 rounded-[3rem] p-10 border-2 border-dashed border-rose-200 shadow-sm">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <h3 className="text-2xl font-black uppercase tracking-tight text-rose-600">Secure Purge</h3>
                      <p className="text-[10px] font-bold text-rose-400 uppercase tracking-widest italic">Terminal Operation</p>
                    </div>
                  </div>
                  ...
                  <div className="p-8 bg-white/60 border border-rose-100 rounded-[2.5rem] flex flex-col sm:flex-row items-center justify-between gap-8">
                    <div className="max-w-md text-center sm:text-left">
                       <p className="font-black text-rose-950 uppercase text-sm mb-2 italic">Global State Erase</p>
                       <p className="text-xs font-bold text-rose-500 leading-relaxed uppercase tracking-tighter">Initiates a complete restoration of system defaults. Current topology and records will be unrecoverable.</p>
                    </div>
                    <button onClick={handleWipeDatabase} className="w-full sm:w-auto px-10 py-5 bg-rose-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-rose-600/30 hover:bg-rose-700 active:scale-95 transition-all">Execute Purge</button>
                  </div>
               </div>
            </div>
          )}
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-0 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300">
            <div className="w-full max-w-4xl h-full sm:max-h-[85vh] bg-white rounded-none sm:rounded-[4rem] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom sm:zoom-in-95 duration-300">
               <div className="p-6 sm:p-10 border-b flex items-center justify-between shrink-0 bg-white">
                  <div>
                    <h3 className="text-2xl sm:text-3xl font-black uppercase tracking-tighter text-slate-900 leading-none">Session Basket</h3>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">{cart.length} active entries in terminal</p>
                  </div>
               ...
               <div className="flex-1 p-4 sm:p-8 overflow-y-auto custom-scrollbar space-y-4 bg-slate-50/30">
                    ...
                    <div key={item.id} className="p-5 sm:p-6 bg-white border border-slate-100 rounded-[2rem] flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 shadow-sm hover:shadow-md transition-shadow">
                       ...
                       <div className="flex items-center justify-between w-full sm:w-auto gap-4">
                          ...
                          <div className="text-right min-w-[120px]">
                            <p className="text-[9px] font-black text-slate-400 uppercase mb-0.5 sm:hidden">Allocation</p>
                  ...
                  {cart.length === 0 && (
                    <div className="py-32 flex flex-col items-center justify-center text-center">
                      <p className="text-slate-400 font-black uppercase tracking-[0.2em] italic text-sm">Session Buffer Empty</p>
                      <button onClick={() => { setIsBasketOpen(false); setActiveTab('Register'); }} className="mt-8 text-blue-600 font-black text-[10px] uppercase tracking-widest hover:underline underline-offset-8 transition-all">Return to Inventory</button>
                    </div>
                  )}
               </div>
               ...
               <div className="p-6 sm:p-10 bg-white border-t flex flex-col sm:flex-row items-center sm:items-end justify-between gap-8 shrink-0">
                  <div className="text-center sm:text-left w-full sm:w-auto">
                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Authorization</p>
            ...
            <div className="w-full max-w-sm bg-white rounded-[3rem] p-10 shadow-2xl print-receipt-card flex flex-col animate-in zoom-in-95 duration-300">
              <div className="text-center mb-8 border-b pb-8">
                ...
                <div className="mt-4 flex justify-between text-[9px] font-black text-slate-400 uppercase tracking-widest">
                  <span>SIG: #{receiptToShow.id}</span>
              ...
              <div className="border-t-2 border-dashed pt-6 mb-10 flex justify-between items-center shrink-0">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Final Settle</span>
              ...
              <div className="flex flex-col gap-3 print:hidden shrink-0">
                <button onClick={() => window.print()} className="w-full py-5 bg-blue-600 text-white rounded-[2rem] font-black text-[10px] uppercase tracking-widest hover:bg-blue-700 active:scale-95 transition-all shadow-xl shadow-blue-600/20">Print Settle Record</button>
                <button onClick={() => setReceiptToShow(null)} className="w-full py-5 bg-slate-900 text-white rounded-[2rem] font-black text-[10px] uppercase tracking-widest hover:bg-slate-800 active:scale-95 transition-all">Close Record</button>
              </div>
            </div>
          </div>
        )}
  ...
  return (
    <div className={`p-8 bg-white border-2 rounded-[2.5rem] transition-all duration-500 flex flex-col justify-between ${alert ? 'border-rose-100 shadow-xl shadow-rose-600/10 animate-pulse' : 'border-slate-50 shadow-sm hover:shadow-xl group'}`}>
      ...
      <div>
        ...
        {alert && <p className="mt-3 text-[8px] font-black text-rose-500 uppercase tracking-[0.2em] italic">Alert: Threshold Breached</p>}
      </div>
    </div>
  );
};
